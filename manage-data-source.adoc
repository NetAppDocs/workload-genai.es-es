...
Puede editar información sobre un origen de datos que haya integrado en una base de conocimientos. La mayor parte de la información se corrige después de agregar un origen de datos a una base de conocimientos, pero puede realizar cambios en la definición de fragmentación y en la detección de permisos (cuando se utiliza el protocolo SMB en el volumen de origen de datos).
...
. En la página Editar origen de datos, seleccione image:button-down-caret.png["el botón de flecha abajo"] para ampliar la fila para la definición de fragmento.
. Actualice la configuración de la estrategia Chunking y el tamaño del fragmento, y la conciencia de permisos (para volúmenes SMB) y seleccione *Guardar*.
